...
void main() {
  runApp(
    const MaterialApp(
      home: Scaffold(body: ChuckNorrisApi()),
    ),
  );
}

class ChuckNorrisApi extends StatefulWidget {
  const ChuckNorrisApi({super.key});

  @override
  State<ChuckNorrisApi> createState() => _ChuckNorrisApiState();
}

class _ChuckNorrisApiState extends State<ChuckNorrisApi> {
  ChuckNorris? chuckNorriData;
  var isLoaded = false;
    ...
    return Column(
      children: [
        ...
        Container(
          ...
          child: Text(
              ' ${chuckNorriData == null ? 'This is a joke' : chuckNorriData?.value}'),
        ),
        const SizedBox(
          height: 20,
        ),
        ...
        ElevatedButton(
          ...
          onPressed: () {
            getData('https://api.chucknorris.io/jokes/random?category=animal');
          },
          child: const Text(
            'Get Animal Jokes',
            style: TextStyle(
  ...
  getData(uri) async {
    chuckNorriData = await RemoteService().getChuckNorrisJoke(uri);
    if (chuckNorriData != null) {
      setState(() {
        isLoaded = true;
      });
    }
